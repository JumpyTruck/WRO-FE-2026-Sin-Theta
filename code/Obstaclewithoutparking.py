...
# cruising speed used for the laps, set once after the parking exit
DRIVE_SPEED = 207
...
SERVO_SETTLE = 0.15    # let the servo reach position before the wheels
                        # start moving that way

# speed used during the parking exit -- separate from DRIVE_SPEED so it
# can be tuned independently (parking usually wants to be gentler)
PARK_SPEED = 100
...
def main():
    ...
    # drive forward at a fixed speed for the rest of the run (the laps) --
    # the Nano holds this direction/speed until told otherwise, so this
    # single call is all that's needed; run() only manages steering.
...
def run(picam2, direction):
    ...
    while True:
        ...
        # ---- lap counting: pure counter, does not touch steering ----
        # (corner/turn hysteresis state machine -- watches only the
        # direction-relevant marker color, masked against the live
        # track polygon)
        ...
        if lap_tracker.quarter_lap_count >= TOTAL_TURNS:
            print(f"[lap] completed {TOTAL_TURNS} quarter-laps, stopping.")
            stop_motor()
            break
